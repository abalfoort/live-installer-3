...
try:
    ...
    from PyQt4.QtGui import QWidget, QFont, QPainter, QPen, QPainterPath, QColor, QPixmap, QApplication
...
class Keyboard(QWidget):
    ...
    def loadCodes(self):
        ...
        cmd = "ckbcomp -model pc106 -layout %s %s -compact | grep ^keycode" % (self.layout, variantParam)
        keycodes = subprocess.check_output(cmd, shell=True).decode('utf-8').strip().split('\n')
        for l in keycodes:
